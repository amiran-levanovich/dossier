...
1. **Session start:** read `goals.md` and `tracker.csv` before responding, and `master_cv_signoff.md` to know the exemplar's state. Never ask what a file answers. All of these live in the current working directory — if `story_bank.md` isn't there, the folder is fresh: route to `job-intake`; never search elsewhere for them.
2. **Order holds:** no `job-apply` without a **signed-off** `master_cv.md` and current goals. A stale sign-off (hash no longer matching the exemplar) counts as unsigned. Route to `job-intake` / `job-goals` first.
3. **Resume, don't re-ask:** `interview_progress.md` is the interview's save-game and its seeded agenda — continue from it; never re-interview recorded areas.
4. **A CV bullet is a hypothesis:** seeded claims live on the agenda until the gauntlet confirms them; only interrogated material enters the story bank, and only the bank backs the exemplar.
5. **Gate before writing:** first the fit gate (`core/fit_check.md` — liveness, constraints kill-switch, evidence-cited score, legitimacy tier; the user's override wins and is recorded), then the ATS keyword check (`standards/ats_rules.md`) — both before building anything.
...
Re-read in this order, stopping as soon as you're oriented: **this file → `interview_progress.md` + `master_cv_signoff.md` → `core/job_workflow.md`**. Still unsure → ask the user. Asking is cheap; guessing wrong is not.

## Minimum read set by task

| Task                     | Read                                                  |
| :----------------------- | :---------------------------------------------------- |
| Intake interview         | `core/interview_protocol.md` + `lifecycle/exemplar.md` |
| Setting goals            | `core/fit_check.md` (how goals feed the gate)          |
| New application          | `core/tailoring_method.md` + `core/fit_check.md` + `standards/ats_rules.md` |
