...
## What this repo is
`dossier` is a **self-hosting Claude Code plugin repo**: `.claude-plugin/marketplace.json` is a single-entry marketplace whose one plugin (`source: "./"`) is this repo itself. Formerly `job-workflow` in the retired `claude_setup` multi-plugin marketplace.

Sibling plugins, same method (discovery → plan → **criteria-first** → produce → review-loop until clean), separate repos:
- [`redgreen`](https://github.com/amiran-levanovich/redgreen) — code (Ruby/Rails & Python)
- [`atelier`](https://github.com/amiran-levanovich/atelier) — non-code deliverables (design, content, research)
...
- **Method changes may want siblings.** The three plugins share a design (a `core/` kernel, `orchestration.md` with an availability check, a `quickref.md` floor, thin skill routers). A structural change here usually wants a parallel PR in `redgreen` and `atelier` — note it in the PR description if you don't do it immediately.
