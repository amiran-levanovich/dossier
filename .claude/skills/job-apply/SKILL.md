...
7. **Dispatch** the `cv-tailor` and `cover-letter-writer` agents in parallel, then run `scripts/trace_check.py` (trace targets resolve?) and the `application-verifier` gate — **fix → re-verify until CLEAN**. Fix and re-verify rounds **continue the same agents via SendMessage** (writer with the findings, verifier with what changed), never fresh launches — each already holds its inputs; relaunch only if a continuation fails or the KB selection changed. Never present documents with open BLOCKER/MAJOR findings.
