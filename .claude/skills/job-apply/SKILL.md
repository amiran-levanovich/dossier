...
3. **The fit gate** (per `job_docs/core/fit_check.md`, same path resolution): liveness + location sanity, binary constraints screen, evidence-cited fit score 1–5 with its band, legitimacy tier — the verdict gets said **before anything is built**. Max 5 WebSearch queries, no sub-agents; the user's override always wins and is recorded in `jd.md` and the tracker.
...
7. **Dispatch** the `cv-tailor` and `cover-letter-writer` agents in parallel, then the `application-verifier` gate — **fix → re-verify until CLEAN**. Never present documents with open BLOCKER/MAJOR findings.
