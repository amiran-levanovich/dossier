...
Read `job_docs/core/kb_schema.md` for the `goals.md` / `constraints.md` contracts. Locate it as follows: use `job_docs/core/kb_schema.md` in the project root if present (drop-in install); otherwise read `../../../job_docs/core/kb_schema.md` relative to this skill's directory (plugin install).

This skill is deliberately small and re-runnable — targets shift during a search; the knowledge base doesn't.

1. **Precondition**: a knowledge base with at least `profile.md` and one verified role. If missing, route to `job-intake` first — goals set against an empty profile are guesses.
